...
var_dump(getRequireORE("FUEL"));

function getRequireORE($name) {
   global $chemicals, $chemicalsMade;

   $ORE = 0;

   foreach ($chemicals[$name]['reactions'] as $reactionName => $qty) {
      if (isset($chemicals[$reactionName])) {

         $chemicalsPerBatch = $chemicals[$reactionName]['qty'];
         $requireORE = 0;

         if (!isset($chemicalsMade[$reactionName])) {
            $chemicalsMade[$reactionName] = [
               'available' => 0
            ];
         }

         while ($chemicalsMade[$reactionName]['available'] < $qty) {
            $requireORE += getRequireORE($reactionName);
            $chemicalsMade[$reactionName]['available'] += $chemicalsPerBatch;
         }
         $chemicalsMade[$reactionName]['available'] -= $qty;
         $ORE += $requireORE;
      } else {
         $ORE += $qty;
      }
