...
func dfs(o map[int64]int64, ip int64, x int64, y int64, coords map[Coord]string) map[Coord]string {

   rcoords := make(map[Coord]string)
   ...
   switch result {
      ...
      case 1:
         ...
         if b {
            ...
            rdfs := dfs(no, nip, x, y + 1, nc)
            if _, e := rdfs[oxyLoc]; e {
               rcoords = rdfs
            }
         }
         break
...
func draw(ocoord map[Coord]string) {
   for cy := maxY; cy >= minY; cy-- {
...
func insertCoord(x int64, y int64, val string) bool {
   c := Coord{
      X: x,
      Y: y,
   }
   _, exists := coord[c]
   // fmt.Println(x, y, exists)
   if exists {
      return false
   }
   coord[c] = val
...
func copyCoordsMap(m map[Coord]string) map[Coord]string {
   t := make(map[Coord]string)
   for k, v := range m {
      t[k] = v
   }
   return t
...
func get(o map[int64]int64, input int64, start int64) (int64, int64) {
